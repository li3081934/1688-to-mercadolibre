...
export async function POST(request: Request) {
  const { text, purpose } = await request.json();
  ...
  const translateModel = models.find((m) => m.purpose === (purpose || "translation"));

  if (!translateModel) {
    return NextResponse.json({ error: "未找到该用途的 AI 模型，请先在 AI 模型管理中配置。" }, { status: 404 });
  }
